...
struct TestRegistrar {
    TestRegistrar(std::string_view testSuiteName, std::string_view testName,
                  const std::function<void()>& fun) {
        // todo: fun rvalue?
        GlobalTestManager::getInstance().add(testSuiteName, std::string{testName}, fun);
    }
};
...
// TODO: lambda not necessary anymore?
...
void registerTyped(Types<Ts...>, const char* testName, const Array& typeNameArray) {
    int typeNameIndex = 0;
    (
        [&] {
            // todo: don't use string
            std::string fullName =
                std::string(testName) + "/" + std::string(typeNameArray[typeNameIndex++]);
            // this isn't full equivalent yet
            Fixture<Ts> instance;
            instance.tstFixtureFun();
            /*tst::printRunTest(fullName.c_str(), []() {
                Fixture<Ts> instance;
                instance.tstFixtureFun();
            });*/
            // todo: errors aren't reset between different typed tests
        }(),
        ...);
}
...
#define TYPED_TEST(FixtureName, TestName)                                                    \
    ...
    const tst::TestRegistrar FixtureName##_##TestName##_Reg(#FixtureName, #TestName, []() {  \
